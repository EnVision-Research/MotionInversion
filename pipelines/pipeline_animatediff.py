...
class VideoDiffPipeline(StableDiffusionControlNetPipeline):
    ...
    def __init__(
        ...
        if motion_adapter is not None:
            unet.config['use_motion_mid_block'] = use_motion_mid_block
            motion_adapter.config['use_motion_mid_block'] = use_motion_mid_block
    ...
    def freeinit(self, latents_video, latents_noise, device, dtype, generator):

        #  initial_noise = randn_tensor(latents.shape, generator=generator, device=device, dtype=dtype)

        current_diffuse_timestep = self.scheduler.config.num_train_timesteps - 1 # diffuse to t=999 noise level

        diffuse_timesteps = torch.full((latents_noise.shape[0],),int(current_diffuse_timestep))
        diffuse_timesteps = diffuse_timesteps.long()

        z_T = self.scheduler.add_noise(
            original_samples=latents_video.to(device), 
            noise=latents_noise.to(device), 
    ...
    def init_filter(
        ...
        height, 
        width, 
        filter_params_method="gaussian",
        filter_params_n=4,
        filter_params_d_s=0.25,
        filter_params_d_t=0.25
    ):
    ...
    def __call__(
        ...
        clip_skip: Optional[int] = None,
        inverse=False,
        frames_video=None, # controlnet_frames
        ...
        # motion_adatper
        if self.unet.__class__.__name__ == 'UNet2DConditionModel':
        ...
        if freeinit:
            latents = self.freeinit(
                latents_video=frames_video,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(self.unet,UNet3DConditionModel):
        ...
        return VideoDiffPipelineOutput(frames=video)
