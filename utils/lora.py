...
class LoraInjectedLinear(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, scale: float = 1.0):
        return (
            ...
            + self.dropout(self.lora_up(self.selector(self.lora_down(hidden_states))))
            * self.scale
        )
...
def inject_trainable_lora_extended(
    ...
    if True:
        for target_replace_module_i in target_replace_module:
            for _module, name, _child_module in _find_modules(
                model, [target_replace_module_i], search_class=[LoRACompatibleLinear, nn.Conv2d, nn.Conv3d]
            ):
                # if name == 'to_q':
                #     continue
                if _child_module.__class__ == LoRACompatibleLinear:
                    weight = _child_module.weight
                    bias = _child_module.bias
                    _tmp = LoraInjectedLinear(
                        _child_module.in_features,
